class Api::V1::RatingsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @rating.save
        ...
        format.json { render :show, status: :created, location: api_v1_rating_url(@rating) }
      else
  ...
  def update
    respond_to do |format|
      if @rating.update(rating_params)
        ...
        format.json { render :show, status: :ok, location: api_v1_rating_url(@rating) }
      else
  ...
  private
    ...
    def rating_params
      params.require(:rating).permit(:recipe_id, :comment, :rate_value).merge(user_id: current_user.id)
    end
end
